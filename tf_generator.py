...
def run_on_test(path_to_test_files,vars_dict, model_path,output_path):
    print("running on test", model_path)
    files = (glob.glob(path_to_test_files + "*.p"))
    files.sort(key=lambda x: int(''.join(filter(str.isdigit, x))))
    path_track = "../track_data/track_data_clean.pickle"

    #model_path = "../test/submissions/model_2/first_run-129"
    #output_path = "../test/submissions/model_2/"


    '''
    #path_to_test_files = "../_validation_test_proc/"
    #path_to_test_files = "../_validation_test_proc/"
    files = (glob.glob(path_to_test_files + "*.p"))
    files.sort(key=lambda x: int(''.join(filter(str.isdigit, x))))
    path_track = "../track_data/track_data_clean.pickle"
...
def train_network(path_to_train,path_to_val,path_track,path_to_res,outer_dict,vars_dict):
    print("training")
    print(path_to_res)
    ...
    while 1:
        ...
        if counter > summary_period:
            ...
            if log_i == 25:
                ...
                print("saved model")
            if (time.time()-current_time)/60/60 > 3.0:
                saver.save(sess, path_to_res + "first_run", global_step=t)
                t += 3
                current_time = time.time()
...
if __name__ == "__main__":
    ...
    #test
        ...
        run_on_test(path_to_test_files, vars_dict, model_path, output_path)